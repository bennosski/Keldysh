...
def compute_U(H, kx, ky, t0, dt, pump, dt_fine, norb):
    ...
    return U
#---------------------------------------------------
def init_Uks(H, dt_fine, myrank, Nkx, Nky, ARPES, kpp, k2p, k2i, tmax, nt, beta, ntau, norb, pump):
    '''
    for ARPES, use Nky = 1 
    '''
    ...
    for ik1 in range(Nkx):
        for ik2 in range(Nky):
            if myrank==k2p[ik1,ik2]:                
                ...
                #prod = np.diag(np.ones(norb, dtype=np.complex128))
                ...
                for it in range(1, nt):
                    t = it*dt # - dt/2.0
                    Ax, Ay = compute_A(t, pump)
                    Ht[index,it] = H(kx-Ax, ky-Ay, t)

                    #prod = np.dot(expm(-1j*H(kx-Ax, ky-Ay)*dt), prod)
                    #prod = np.dot(expm(-1j*Ht[index,it]*dt), prod)

                    #UksR[index,it] = expm(-1j*H(kx-Ax, ky-Ay, t)*dt) @ UksR[index,it-1]
                    
                    '''
                    print('H')
                    print(H(kx-Ax,ky-Ay,t))
                    print('expm')
                    print(expm(-1j*H(kx-Ax, ky-Ay, t)*dt))
                    print('UksR')
                    print(UksR[index,it])
